...
export async function GET(req: Request) {
  ...
  const threads = listThreads(forumId).map((t) => ({ ...t, posts: listPosts(t.id) }));
...
export async function POST(req: Request) {
  ...
  const member = userId ? isMember(forumId, userId) : false;
  ...
  const thread = createThread({ forumId, title, authorId: userId });
  const post = createPost({ threadId: thread.id, authorId: userId, contentHtml });
